...
var autoBattle = {
    ...
    getFreshRings: function(){
        return {
            ...
            seed: {
                bleed: [Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9)],
                poison: [Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9)],
                shock: [Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9), Math.floor(Math.random() * 1e9)],
            }
        };
    },
    resetAll: function(){
    ...
    settings: {
        ...
        practice: {
            enabled: 0,
            default: 0,
            text: ["Practice Off - 10x Speed, <strike>no Dust or Progress</strike>", "Practice On - 10x Speed, <strike>no Dust or Progress</strike>"],
            onToggle: function(){
    ...
    items: {
        ...
        Goo_Golem: {
            ...
            doStuff: function(){
                ...
                if (now > autoBattle.trimp.lastGoo + 1000){                 
                    if (autoBattle.trimp.lastGoo == -1) autoBattle.trimp.lastGoo = now;
        ...
        Basket_of_Souls: { //after all additive lifesteal and health (before monkimp)
            ...
            upgrade: "+1000 Health, +300 Defense, +30% Lifesteal, +200% Bleed Damage, +200% Shock Damage",
            health: function(){
                return 2000 + (1000 * this.level);
            },
            defense: function(){
        ...
        Doppelganger_Signet: { //actual final attack item
            ...
            zone: 190,
            description: function(){
                return "Summon a Doppelganger which grants you 50% damage reduction, 2x Attack, and +1 Poison Stack Rate while it is alive. Your Doppelganger will explode after taking damage equal to your Max Health, redealing all damage dealt so far this fight, and shredding 50% Enemy Defense.";
            },
            onDeath: function(){
                var damageDealt = autoBattle.enemy.dmgTaken;
                autoBattle.damageCreature(autoBattle.enemy, damageDealt);
            ...
            doStuff: function(){
                ...
                if (autoBattle.trimp.dmgTaken >= autoBattle.trimp.maxHealth) this.onDeath();
            },
            noUpgrade: true,
            dustType: "shards"
        },
    ...
    checkItems: function(){
        ...
        for (var ring in this.rings.equipped){
            var ringObj = this.rings.equipped[ring];
            for (var mod in ringObj.stats){
                var modObj = this.ringStats[mod];
                modObj.doStuff(ringObj.stats[mod]);
            }
        }
        for (var item in this.items){
            var itemObj = this.items[item];
    ...
    setProfile: function(){
        ...
        var doubleResist = getRandomIntSeeded(seed++, 0, 100);
        ...
        for (var x = 0; x < effectsCount; x++){
            ...
            switch(effect){
                ...
                case "Explosive":
                    ...
                    if (count == 1) {
                        this.enemy.explodeDamage = 1.2;
                        this.enemy.explodeFreq = 20000;
                    }
                    else {
                        this.enemy.explodeDamage += 0.2;
    ...
    checkLastActions: function(){
        ...
        for (var x = 0; x < this.lastActions.length; x++){
            console.log(prettify(this.lastActions[x][2]), prettify(this.dust), prettify(this.lastActions[x][6]), prettify(this.shards));
    ...
    saveLastAction: function(type, what, cost){
        var useShards = false;
        if (type == "contract" || type == "cancelContract" && this.items[what].dustType == "shards") useShards = true;
    ...
    getRingTierName: function(tier){
        switch(tier){
            case 0: return "Runed";
            case 1: return "Polished";
            case 2: return "Sparkling";
            default: return "Empty";
        }
    },
    getRingTierPrice: function(tier){
        switch(tier){
            case 0: return 10;
            case 1: return 1000;
    ...
    getRingUi: function(){
        var text = "<div style='color: white; padding: 0.5%;'>Here you can craft your Shards into Jewels to socket in Huffy's Rings! You can have 1 Bleed, Shock, and Poison Jewel equipped at a time.</div>";
        text += "<div class='ringContainer'><span style='padding: 0.5%'>Equipped</span><br/>";
        for (var ring in this.rings.equipped){
            var ringObj = this.rings.equipped[ring];
            text += "<div class='equippedRing'>" + this.getRingTierName(ringObj.tier) + " " + ring.charAt(0).toUpperCase() + ring.slice(1) + "<br/>";  
            for (var stat in ringObj.stats){
                var mod = this.ringStats[stat];
                text += mod.name(ringObj.stats[stat]) + "<br/>";
            }
            text += "</div>";
        }
        text += "</div>";
        text += "<div class='ringContainer'><span style='padding: 0.5%'>Inventory</span><br/>";
        for (var ring in this.rings.rolled){
            var ringObj = this.rings.rolled[ring];
            text += "<div class='rolledRing'>" + this.getRingTierName(ringObj.tier) + " " + ring.charAt(0).toUpperCase() + ring.slice(1) + "<br/>";
            
            for (var stat in ringObj.stats){
                var mod = this.ringStats[stat];
                text += mod.name(ringObj.stats[stat]) + "<br/>";
            }
            if (ringObj.tier < 2) text += "<br/>";
            text += "<div style='text-align: center'>"
            if (this.confirmEquipRing == ring){
                text += "<span style='font-size: 0.75vw'>Are you sure? This will destroy your currently equipped " + ring.charAt(0).toUpperCase() + ring.slice(1) + " Jewel.</span><br/>";
                text += "<span style='font-size: 0.6vw; padding: 0.5em; width: 40%' class='btn btn-sm autoItemEquipped' onclick='autoBattle.equipRing(\"" + ring + "\")'>Equip</span><span style='font-size: 0.6vw; padding: 0.5em; width: 40%' class='btn btn-sm autoItemHide' onclick='autoBattle.cancelEquipRing()'>Cancel</span>"
            }
            else{
                text += "Reroll:<br/>"
                for (var x = 0; x < 3; x++){
                    text += "<span class='btn btn-sm autoItemUpgrade' onclick='autoBattle.createRing(\"" + ring + "\", " + x + ")'>" + this.getRingTierName(x) + "<br/>" + prettify(this.getRingTierPrice(x)) + " Shards</span>";
                }
                if (ringObj.tier >= 0) text += "<br/><span style='font-size: 0.6vw; padding: 0.5em; width: 100%;' class='btn btn-sm autoItemEquipped' onclick='autoBattle.checkEquipRing(\"" + ring + "\")'>Equip</span>";
            }
            text += "</div></div>";
        }
        text += "</div>";
    ...
    confirmEquipRing: null,
    cancelEquipRing: function(){
        this.confirmEquipRing = null;
        this.popup(false, false, true);
    },
    checkEquipRing: function(type){
        if (autoBattle.rings.equipped[type].tier >= 0) {
            this.confirmEquipRing = type;
            this.popup(false, false, true);
            return;
        }
        this.equipRing(type);        
    },
    equipRing: function(type){
        this.confirmEquipRing = null;
        this.rings.equipped[type] = this.rings.rolled[type];
        this.rings.rolled[type] = {};
        this.rings.rolled[type].stats = {};
        this.resetCombat();
        this.resetStats();
        this.popup(false, false, true);
    },
    createRing: function(type, tier){
        var cost = this.getRingTierPrice(tier);
        if (this.shards < cost) return;
        this.shards -= cost;
        var seed = this.rings.seed[type][tier];
        var spMods = [];
        var neutralMods = [];
        var selectedMods = {tier: tier, stats: {}};
        for (var item in this.ringStats){
            if (this.ringStats[item].type == type) spMods.push(item);
            else if (this.ringStats[item].type == "neutral" && this.ringStats[item].excl != type) neutralMods.push(item);
        }
        var rollType = getRandomIntSeeded(seed++, 0, spMods.length);
        var mod = this.ringStats[spMods[rollType]];
        var numRoll = getRandomIntSeeded(seed++, mod.rolls[tier][0], mod.rolls[tier][1]);
        selectedMods.stats[spMods[rollType]] = numRoll;
        var neutRolls = (tier == 2) ? 2 : 1;
        for (var x = 0; x < neutRolls; x++){
            rollType = getRandomIntSeeded(seed++, 0, neutralMods.length);
            mod = this.ringStats[neutralMods[rollType]];
            numRoll = getRandomIntSeeded(seed++, mod.rolls[tier][0], mod.rolls[tier][1]);
            selectedMods.stats[neutralMods[rollType]] = numRoll;
            neutralMods.splice(rollType, 1);
        }
        this.rings.seed[type][tier] = seed;
        this.rings.rolled[type] = selectedMods;
        this.popup(false, false, true);
    },
    ringStats: {
            bleedMod: {
                name: function(amt){
                    return "+ " + prettify(amt) + "% Bleed Damage";
                },
                type: "bleed",
                doStuff: function(amt){
                    amt /= 100;
                    autoBattle.trimp.bleedMod += amt;
                },
                rolls: [[100,200], [250,350], [500,750]]
            },
            bleedChance: {
                name: function(amt){
                    return "+ " + prettify(amt) + "% Bleed Chance";
                },
                type: "bleed",
                doStuff: function(amt){
                    autoBattle.trimp.bleedChance += amt;
                },
                rolls: [[25,50], [50,100], [100,200]]
            },
            shockMod: {
                name: function(amt){
                    return "+ " + prettify(amt) + "% Shock Damage";
                },
                type: "shock",
                doStuff: function(amt){
                    amt /= 100;
                    autoBattle.trimp.shockMod += amt;
                },
                rolls: [[100,200], [250,350], [500,750]]
            },
            shockChance: {
                name: function(amt){
                    return "+ " + prettify(amt) + "% Shock Chance";
                },
                type: "shock",
                doStuff: function(amt){
                    autoBattle.trimp.shockChance += amt;
                },
                rolls: [[25,50], [50,100], [100,200]]
            },
            poisonMod: {
                name: function(amt){
                    return "+ " + prettify(amt) + " Poison Damage";
                },
                type: "poison",
                doStuff: function(amt){
                    autoBattle.trimp.poisonMod += amt;
                },
                rolls: [[10,20], [40,80], [100,200]]
            },
            poisonChance: {
                name: function(amt){
                    return "+ " + prettify(amt) + "% Poison Chance";
                },
                type: "poison",
                doStuff: function(amt){
                    autoBattle.trimp.poisonChance += amt;
                },
                rolls: [[25,50], [50,100], [100,200]]
            },
            attack: {
            ...
            health: {
                ...
                rolls: [[250, 500], [500,750],[1000,1250]]
            },
            defense: {
                name: function(amt){
    ...
    popup: function(updateOnly, statsOnly, itemsOnly, leaveMode){
        ...
        for (var x = 0; x < things.length; x++){
            ...
            else if ((fighterObj.poisonChance > 0 && (!this.rings.equipped.poison.stats.poisonChance || this.rings.equipped.poison.stats.poisonChance < fighterObj.poisonChance)) || fighterObj.poisonTime > 0 || (fighterObj.poisonMod > freePmod && (!this.rings.equipped.poison.stats.poisonMod || this.rings.equipped.poison.stats.poisonMod + freePmod < fighterObj.poisonMod))){
            ...
            else if ((fighterObj.bleedChance > 0 && (!this.rings.equipped.bleed.stats.bleedChance || this.rings.equipped.bleed.stats.bleedChance < fighterObj.bleedChance)) || fighterObj.bleedTime > 0 || (fighterObj.bleedMod > 0 && (!this.rings.equipped.bleed.stats.bleedMod || this.rings.equipped.bleed.stats.bleedMod / 100 < fighterObj.bleedMod))){
                statsText += "<span class='abError'>"
            ...
            else if ((fighterObj.shockChance > 0 && (!this.rings.equipped.shock.stats.shockChance || this.rings.equipped.shock.stats.shockChance < fighterObj.shockChance)) || fighterObj.shockTime > 0 || (fighterObj.shockMod > 0 && (!this.rings.equipped.shock.stats.shockMod || this.rings.equipped.shock.stats.shockMod / 100 < fighterObj.shockMod))){
                statsText += "<span class='abError'>"
        ...
        if (this.maxEnemyLevel > 50) text += "<span onclick='autoBattle.swapPopup(\"rings\")' class='btn btn-lg autoColorTeal active darkBorder'>Rings</span>";
        ...
        // if (this.popupMode == "rings" && itemsElem) itemsElem.style.maxHeight = "19em";
        // else if (itemsElem) itemsElem.style.maxHeight = "11em";
        if (scrollTop > 0){
